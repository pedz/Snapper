 ...
 */
/**
 * @see https://www.w3.org/TR/SVG/struct.html#SVGElement
 ...
 */
/**
 * @see https://www.w3.org/TR/SVG/coords.html#InterfaceSVGPoint
...
(
	...
	/**
	 * Helper function to add a title element so an SVG element.
	 * @param {SVGTree} svg - An SVG element to prepend the title
	 * element to.
	 * @param {String} text - The text (tooltip) to add to the
	 * title element
	 * @return {SVGTree} Returns the svg element passed in to
	 * allow for chaining.
	 */
	function addTitle(svg, text) {
	    svg.prepend(createElement('title').append(text));
	...
	function connectListToTarget(container, target, list, target_index, target_count) {
	    ...
	    /*
	     * Add the Port VLAN ID and the VLAN Tag IDs as classes to the polyline.
	     */
	...
	/**
	 * Creates the appropriate type of ethernet adapter which may
	 * be an {@link Snapper~EthChan EthChan} or an {@link
	 * Snapper~Ent Ent}.
	 * @param {Snapper~LPAR} lpar - The LPAR that contains the
	 * ethernet adapter.
	 * @param {Snapper~Device} device - The device which is the
	 * ethernet adapter.
	 */
	function createEnt(lpar, device) {
	    if (device.isEthChan()) {
	...
	World.prototype = {
	     ...
	     * world which ends up calling {@link Snapper~CEC#draw}
	     * and placing them into an SVG group element with `id` of
	     * `World` and `class` of `world`.  @return {SVGTree}
	     */
		...
		function func() {
			...
			var p = $('<p class="error">');
			div.append($('<h2 class="error">').append("We encountered a javascript error"))
			    .append(p);
			p.append(e.message)
			    .append($("<br/>"))
			    .append(e.name)
			    ...
			    .append($("<br/>"))
			    .append(e.lineNumber);
			e.stack.split("\n").forEach(function (l, i, c) {
			    p.append($('<br/>')).append(l);
			});
			$('body').append(div);
		    }
		};
	...
	CEC.prototype = {
	     ...
	     * with {@link Snapper~CEC#lpars lpars} which ends up
	     * calling {@link Snapper~LPAR#draw} and placing them into
	     * a group element with an `id` of the CEC's {@link
	     * Snapper~CEC#name name} and a `class` of `cec`.  @return
	     * {SVGTree}
	     */
	...
	function LPAR(cec, id, snap) {
	    ...
	    /**
	     * The interfaces within the LPAR.  We find all devices
	     * with a uniquetype of the if/EN/en (which ignores token
	     * ring and all other types of interfaces) which have a
	     * customized netaddr.  @type {Array.<Snapper~Ifnet>}
	    ...
	    /**
	     * We run through the {@link Snapper~LPAR#ifnets
	     * interfaces} and filter out the SEAs and are left with
	     * non-SEA adapters with an interface.  @type
	     * {Array.<Snapper~Ent|Snapper-EthChan>}
	     */
	    this.adapters = this.ifnets.filter(function (ifnet) {
	    	return !(ifnet.adapter.isSEA());
	    }).map(function (ifnet) {
	    	var ent = createEnt(this, ifnet.adapter);
	...
	LPAR.prototype = {
	     ...
	     * createInterconnects} for each SEA.
	     */
	     ...
	     * Snapper~LPAR#adapters adapters} and also renders the
	     * interfaces for those adapters.  All of these are put
	     * into a group element with an `id` of the LPAR's {@link
		...
		if (this.seas) {
			...
			this.ifnets.forEach(function (ifnet) {
				...
				ifnetSVG = ifnet.svg;
				ifnetBBox = ifnetSVG[0].getBBox();
				adapterSVG = ifnet.adapter.ent.svg;
				adapterBBox = adapterSVG[0].getBBox();
				lparSVG = g[0];

				/*
				 * This is a bit confusing.  We are
				 * about to translate the SEA by x and
				 * y *after* adding the width of the
				 * interface plus the padding.  The
				 * other thing that is confusing is
				 * for p1.x, we don't really want it
				 * relative to the adapter's left edge
				 * but rather the SEA's left edge --
				 * which is equal to x before we add
				 * on the width of the interface plus
				 * padding.
				 */
				p1 = svgElement.createSVGPoint();
				p2 = svgElement.createSVGPoint();
				p3 = svgElement.createSVGPoint();
				p1.y = y;
	 ...
	 * "interface" is reserved in Javascript so I went with Ifnet.
	 *
	 * @constructor
	 *
	...
	Ifnet.prototype = {
	    /**
	     * The ifnet is represented by an isosceles triangle
	     * pointing down.  The top is side size in pixels and the
	     * height is side size.
	     *
	     * @constant {Number}
	     *
	    ...
	    addTitle: function addTitleForIfnet(g) {
		var netstat_in;
		var ifconfig_a;
		var hash;
		var title = undefined;

		if ((hash = this.device.hash)) {
		    netstat_in = hash.netstat_in;
		    ifconfig_a = hash.ifconfig_a;
		}

		if (ifconfig_a) {
		    ifconfig_a.addrs.forEach(function (addr) {
			if (addr.family == "ipv4") {
			    if (title)
				title += "\n";
			    title = addr.address + "/" + addr.netmask_length.toString(10);
			}
		    }, this);
		}
		if (netstat_in) {
		    [ 'mac', 'mtu', 'ipkts', 'ierrs', 'opkts', 'oerrs' ].forEach(function (attr) {
			var value = netstat_in[attr];
	...
	function Device(lpar, device, snap) {
		...
		/**
		 * If the device is an ethernet adapter, SEA, or
		 * etherchannel then this will be defined.
		 *
		 * @type {undefined|Snapper~Ent|Snapper~SEA|Snapper~EthChan}
		 */
		this.ent = undefined;
		...
		e.message = e.message + " Device:" + device.name;
		throw e;
	    }
	...
	function SEA(lpar, device) {
	    ...
	    /**
	     * The id of the SEA which is the lpar name plus the
	     * device name.
	     *
	     * @type {String}
	     */
	     ...
	     */
	    [ 'ctl_chan', 'real_adapter', 'virt_adapters' ].forEach(function (attr_name) {
		var attr;
		var value;

		attr = device.attributes[attr_name];
		value = attr ? attr.value : '';
		if (value == '')
		    this[attr_name] = undefined;
		else
		    this[attr_name] = value.split(',').map(function (devName) {
			var device = this.lpar.devices.byName[devName];
			if (typeof device === 'undefined')
			    alert("SEA ctor: device '" + devName + "' not found in parent LPAR " + lpar.name);
			return createEnt(this.lpar, device);
		    }, this);
	    }, this);
	...
	function EthChan(lpar, device) {
	    ...
	    /**
	     * The id of the etherchannel which is the lpar name plus
	     * the device name.
	     *
	     * @type {String}
	     */
	     ...
	     */
	    [ "adapter_names", "backup_adapter" ].forEach(function (attr_name) {
		var attr;
		var attr_value;
		...
		if (attr_value == '')
		    ...
		    this[attr_name] = attr_value.split(',').map(function (devName) {
		    if (devName == "NONE") {
			return undefined;
		    }
		    var device = this.lpar.devices.byName[devName];
	...
	Ent.prototype = {
	    ...
	    addTitle: function addTitleForEnt(g) {
		var hash = this.device.hash;
		var title;

		title = hash.cu_dv.ddins;
		addTitle(g, title);
	    },
	...
	function Attribute(cu_ats, pd_ats) {
	    ...
	    if (!pd_ats)
		pd_ats = [];
	    if (cu_ats.length === undefined)
		throw new TypeError("cu_ats is not an array");
	    if (pd_ats.length === undefined)
	...
	if (typeof window.snapper === "undefined") window.snapper = {};
	window.snapper.Snapper = Snapper;

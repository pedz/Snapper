...
(
	...
	function LPAR(cec, id, data) {
		...
		var newDevice = new Device(this.lpar, cudv, this.snap);
		...
		var addr = (device.attributes['netaddr'].values.length > 0);
		var dev = this.devices.byName[device.name.replace(/e[nt]/, 'ent')];
		return (addr && dev);
	    }, this).map(function (device) {
	...
	Ent.prototype = {
		...
		if (this.entstat)
		    this.commandList.add("entstat", this.entstat);

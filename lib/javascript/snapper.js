 ...
 */
/**
 * @desc The Ruby code has a Device class which the Javascript code
 * uses to some degree.
 * @typedef {Object} RubyDevice
 */
/**
 * @desc The Ruby code has a Batch class which is a list of Snap
 * instances along with a list of Alert instances.
...
(
	...
	function World() {
	    ...
	    /**
	     * This array is for the batches that are added.
	...
	World.prototype = {
	    ...
	    /**
	     * Batch is defined in the Ruby code.  We ignore the
	     * alerts for now and walk the snap_list.  The snap_list
	     * is a list of snaps that we will add via {@link
	     * Snapper~World#addSnap addSnap}.
	     *
	     * @param {Batch} batch
	     */
	    addBatch: function addBatchToWorld(batch) {
		batch.snap_list.forEach(World.prototype.addSnap, this);
	    },
	     ...
	     * CEC} is created and its {@link Snapper~CEC#addSnap
	     * addSnap} method is called with the same data passed
	     * into this function.
	     *
	...
	function LPAR(cec, id, snap) {
		...
		if (device.pddv) {
		    ...
		    var thisType = this.devices.byType[device.cudv.pddvln];

		    this.devices.byName[device.cudv.name] = newDevice;
	    	    if (typeof thisType === 'undefined')
	    		thisType = (this.devices.byType[device.cudv.pddvln] = []);
	...
	function Device(lpar, device, snap) {
	    ...
	    var cudv = device.cudv;
	    var uniquetype = device.pddv.uniquetype;

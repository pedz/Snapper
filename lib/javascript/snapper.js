...
(
	...
	CEC.prototype = {
		 ...
		 */
		Object.getOwnPropertyNames(this.virtual_switches).forEach(function (virtual_switch_name) {
		    var cec_svg = this.svg[0];
		    var virtual_switch = this.virtual_switches[virtual_switch_name];
		    var from_rect = get_rect(virtual_switch);;
		    var from_bbox = from_rect.getBBox();
		    var from_transform = cec_svg.getTransformToElement(from_rect);
		    var from_point = svgElement.createSVGPoint();
		    var from_x;
		    var from_y;
		    var delta_x;
		    ...
		    /*
		     * above_me and below_me will be arrays of ent
		     * elements (both subsets of connections) that
		     * will be split into above or below based upon if
		     * their y position is above or below the virtual
		     * switch ("me") and then sorted based upon their
		     * x position.
		     *
		    ...
		    virtual_switch.connections.forEach(function (ent) {
			var to_rect = get_rect(ent);
			var to_bbox = to_rect.getBBox();
			var to_transform = cec_svg.getTransformToElement(to_rect);
			var obj = {
			    rect: to_rect,
			    bbox: to_bbox,
			...
			/*
			 * to_transform is starting from the from_rect
			 * (the virtual switch) and going to the
			 * to_rect (the ethernet adapter).  Thus a
			 * negative f value is when the ethernet
			 * adapter is below (further down the page)
			 * from the virtual switch ("me").
			 */
			if (from_transform.f > to_transform.f)
			    below_me.push(obj);
			else
			    above_me.push(obj);
		    }, this);
		    var compare = function (l, r) {
			return r.transform.e - l.transform.e;
		    };
		    below_me = below_me.sort(compare);
		    above_me = above_me.sort(compare);

		    /*
		     * for below_me, start at bottom left corner
		     */
		    from_x = 0;
		    delta_x = from_rect.width.baseVal.value / (below_me.length + 1);
		    from_y = from_rect.height.baseVal.value;
		    below_me.forEach(function (obj) {
			var to_x = obj.rect.width.baseVal.value / 2;
			var to_y = 0;
			var blah;

			// first = false;
			from_x += delta_x; //move across edge

			to_point.x = to_x;
			to_point.y = to_y;
			to_point = to_point.matrixTransform(obj.transform.inverse());
			from_point.x = from_x;
			from_point.y = from_y;
			from_point = from_point.matrixTransform(from_transform.inverse());
			blah = line(from_point, to_point);
			this.svg.append(blah);
		    }, this);

		    from_x = 0;
		    delta_x = from_rect.width.baseVal.value / (above_me.length + 1);
		    from_y = 0;
		    above_me.forEach(function (obj) {
			var to_x = obj.rect.width.baseVal.value / 2;
			var to_y = obj.rect.height.baseVal.value;
			var blah;
		...
		layout.padding = this.borderWidth;

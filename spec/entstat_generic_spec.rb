...
EOF
    ...
    it "parses the hardware MAC address" do
      expect(@result["Hardware Address"]).to eq("98:be:94:67:f5:14")
    end
    it "parses Actor State" do
      expect(@result["Actor State"]["LACP activity"]).to eq("Active")
    end
    it "parses Partner System" do
      expect(@result["Partner System"]).to eq("00-23-04-EE-BE-9E")
    end
  end
end
